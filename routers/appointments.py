...
async def get_appointments(
    ...
    print(resource_id)
    print(patient_id)
    if resource_id is None and patient_id is None:
        appointments = await Appointment.all()
    elif resource_id is not None and patient_id is None:
        appointments = await Appointment.filter(resource_id=resource_id, demand__patient_id=patient_id)
    if patient_id is not None:
        appointments = await Appointment.filter(demand__patient_id=patient_id)
    else:
        appointments = await Appointment.filter(resource_id=resource_id)
